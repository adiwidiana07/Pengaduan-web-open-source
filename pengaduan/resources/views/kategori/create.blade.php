...
@section('title', 'Tambah Kategori - Dashboard')
...
<div class="max-w-2xl">
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        <div class="p-6 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-500 mt-1">Buat kategori baru untuk pengelompokan aspirasi.</p>
        </div>

        <form action="{{ route('kategori.store') }}" method="POST" class="p-6 space-y-6" data-loading>
            @csrf
            <div>
                <label for="nama_kategori" class="block text-sm font-semibold text-gray-700 mb-2">Nama Kategori</label>
                <input type="text" name="nama_kategori" id="nama_kategori" value="{{ old('nama_kategori') }}" required maxlength="255"
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3" maxlength="500" data-auto-resize
                    class="w-full px-4 py-2.5 border @error('deskripsi') border-red-300 focus:border-red-500 focus:ring-red-500 @else border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 @enderror rounded-lg text-sm text-gray-900 focus:outline-none focus:ring-1 transition-colors">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
